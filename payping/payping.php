...
class payping extends PaymentModule {
	...
	public function do_payment($cart) {
		...
		if ( ! isset( $_POST['orderId']) ){
			echo $this->error($this->l('There is a problem.').' secure_key is wrong !' );
			return ;
		} else {
			try {
				$dataExploade = explode('~',$_POST['orderId']);
				if ( md5 ($dataExploade[0] .' Erfan Ebrahimi 09361090413'. Configuration::get('payping_HASH_KEY') . $dataExploade[1] ) != $dataExploade[2]  ){
					echo $this->error($this->l('There is a problem.').' secure_key is wrong !' );
					return ;
				}
			}catch (Exception $e){
				echo $this->error($this->l('There is a problem.').$e->getMessage());
				return ;
			}
		}
		$amount = floatval(number_format($dataExploade[1], 0, '', '')/$currency);
		$callbackUrl = $this->l('link') . 'modules/payping/pfunction.php?do=call_back&id=' . $dataExploade[0] . '&amount=' . $amount. '&hash=' . md5($amount.$dataExploade[0].Configuration::get('payping_HASH_KEY'));
		$orderId = $dataExploade[0];
	...
	public function hookPayment($params) {
		global $smarty;
		$smarty->assign('orderId' , $this->context->cart->id .'~'. $this->context->cart ->getOrderTotal(true, 3) .'~' . md5($this->context->cart->id .' Erfan Ebrahimi 09361090413'. Configuration::get('payping_HASH_KEY') . $this->context->cart ->getOrderTotal(true, 3)) );
